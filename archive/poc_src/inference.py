...
def load_class_exemplars():
    ...
    class_exemplars = {0: "", 1: "", 2: "", 3: ""}  # Default empty for classes without rules
...
def run_multiclass_inference(model, texts, class_exemplars, device):
    """Run multiclass inference: compute similarity to each class exemplar pool."""
    model.eval()
    predictions = []
    all_similarities = []  # For each text, list of 4 similarities (one per class)

    class_fallbacks = {
        0: "neutral allowed content",
        1: "offensive violent threatening language",
        2: "mature sexual drug self-harm content",
        3: "algospeak coded evasion language",
    }
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(texts), 32), desc="Multiclass Inference"):
            ...
            # Compute similarities to each class exemplar pool
            batch_sims = []
            for cls in [0, 1, 2, 3]:
                exemplar = class_exemplars[cls] or class_fallbacks[cls]
            ...
            # batch_sims is list of 4 arrays, each of shape (batch_size,)
            for j in range(len(batch_texts)):
                text_sims = [batch_sims[cls][j] for cls in range(4)]
                all_similarities.append(text_sims)
                pred_class = int(text_sims.index(max(text_sims)))  # Argmax over 4 classes
...
def main():
    ...
    conf_matrix = confusion_matrix(labels, predictions, labels=[0, 1, 2, 3])
    logger.info("Confusion Matrix:")
    header = f"{'':12}" + "".join(f"{n:12}" for n in class_names)
    logger.info(f"Predicted → {header}")
    for i, row in enumerate(conf_matrix):
        row_str = "".join(f"{v:12}" for v in row)
        logger.info(f"  {class_names[i]:10} {row_str}")

    logger.info("\nClassification Report:")
    report = classification_report(labels, predictions, target_names=class_names, digits=4, labels=[0, 1, 2, 3])
    logger.info(report)
    ...
    multiclass_df = pd.DataFrame({
        ...
        'sim_algospeak': [s[3] for s in all_similarities],
    })
