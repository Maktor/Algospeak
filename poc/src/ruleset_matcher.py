...
class RulesetMatcher:
    ...
    def __init__(self, rules_jsonl_path):
        ...
        self.rules_by_class = {0: [], 1: [], 2: []}  # Track rules per class (0/1/2)
    ...
    def _load_rules(self, rules_path):
        ...
        # If there are no class-0 rules, we won't have any allowed exemplars
        if len(self.rules_by_class[0]) == 0:
            logger.warning("No class-0 (Allowed) rules found – "
                           "texts with no fired rules will get an empty exemplar string")
    ...
    def get_fired_rules(self, text):
        ...
        for rule_id, rule_info in self.rules.items():
            ...
            # Check if pattern is a substring of text (simple keyword matching)
    ...
    def get_exemplars_for_text(self, text):
        ...
        If no rules fire, use class-0 (Allowed) exemplars as fallback.
        ...
        # Step 2: Collect exemplars from fired rules.  If nothing matches we fall back
        # to the allowed-class (0) exemplars; if even those are missing we simply
        # return an empty string so the rule encoder sees nothing.
        if not fired_rules:
            # Fallback: use class-0 exemplars if any
            exemplar_list = []
            for rule_id in self.rules_by_class[0]:
